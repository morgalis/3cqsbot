...
def account_data():
    ...
    if error:
    ...
    elif config['trading'].getboolean('use_accounts_id') == False:
        ...
        for accounts in data:
            ...
            #logging.info(str(accounts['id'])+ ' '+accounts['name'])
        ...
        for accounts in data:
            accounts_listed.append({'id': str(accounts['id']),'market_code': str(accounts['market_code'])})

        return accounts_listed
...
async def my_event_handler(event):
    ...
    if (asyncState.btcbool and
        ...
        accountids = json.loads(config.get("trading", "accounts_ids"))
        for account in accountids:
            for acc in account_output:
                if str(account) == acc['id']:
                    logging.info(f"New signal on accound Nr: {acc['id']}")
                    pair_output = pair_data(acc)
                    if tg_output and not isinstance(tg_output, list):
                        if config['dcabot'].getboolean('single'):
                            bot = SingleBot(tg_output, bot_output, acc, config, p3cw, logging)
                        else:

                            bot = MultiBot(tg_output, bot_output, acc, pair_output, config, p3cw, logging)
                        ...
                        if (tg_output['volatility'] != 0 and
                            ...
                            logging.info("Token " + tg_output['pair'] + " is not traded on account " + str(acc['id']))
